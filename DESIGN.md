...
отметка на диске раньше пробы на 5-й секунде. Чтобы вообще что-то сдвинуть, запрос должен занять
больше ~28 с, а дольше 60 с он занять не может — это таймаут aiogram, одна попытка. Худший
УСПЕШНЫЙ старт: отметка на ~62 с, `healthy` на пробе +90 с, то есть ~30 с запаса до окна апдейтера.

Важная деталь этого порядка: провалившийся первый запрос обязан УБИТЬ процесс, а не подвесить его.
Соединение aiosqlite живёт в НЕ-daemon потоке, поэтому незакрытая база держит интерпретатор живым
после того, как трейсбек уже напечатан (измерено: контейнер висел 200+ с). Поэтому в `main.py` всё,
что может упасть после `db.init()`, стоит внутри `try`, а `finally` закрывает базу и сессию бота.
